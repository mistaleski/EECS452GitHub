...
;initialization
	...
	BCLR	FRCT			;Do not shift output of multiplier left by one bit
...
;  XAR7: reinit XAR1
	...
	MPYM	*AR1+, *AR0, AC0			; AC0 = b0*x(n)
	MACM	*AR1+, *AR3+, AC0		; AC0 += b1*x(n-1)
	MACM	*AR1+, *AR3-, AC0		; AC0 += b2*x(n-2)
	MASM	*AR1+, *AR4+, AC0		; AC0 -= a0*y(n-1)
	...
	MOV *AR0 << #16, AC1
	MOV	HI(AC1), *AR3		; x(n-1) replaces x(n-2)
	||AADD	T0, AR3			;point to x(n)

	MOV *AR0 << #16, AC1
	MOV	HI(AC1), *AR3		; x(n) replaces x(n-1)
	||AADD	T0, AR3			;point to next x(n-1)
