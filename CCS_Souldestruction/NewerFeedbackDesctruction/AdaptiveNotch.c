...
void drawNotches()
{
	...
	for(i=0; i< NUM_CANCELERS; i++)
	{
		...
		{
			GoTo(filterCenter[i]*2-_NOTCH_LINE_SIZE+FFT_DRAW_OFFSET,(_FEEDBACK_THRESHOLD >>4)-_FEEDBACK_FREQUENCY_TOLERANCE);
			Draw(BLUE,filterCenter[i]*2 +_NOTCH_LINE_SIZE+FFT_DRAW_OFFSET, (_FEEDBACK_THRESHOLD >>4)-_FEEDBACK_FREQUENCY_TOLERANCE);
		}
	}
}
